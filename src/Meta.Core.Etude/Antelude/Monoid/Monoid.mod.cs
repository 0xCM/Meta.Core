    ...
    public class Monoid : ClassModule<Monoid, IMonoid>, IMonoid
    {
        ...
        public static Option<IMonoid<X>> make<X>()
            => Try(() => Instances.TryFind(typeof(X))
                        .MapValueOrDefault(instance => cast<IMonoid<X>>(instance), MonoidI<X>.Default));

        static ConcurrentDictionary<Type, object> Instances { get; }
            = new ConcurrentDictionary<Type, object>();

        /// <summary>
        /// Constructs a list monoid
        /// </summary>
        /// <typeparam name="X"></typeparam>
        /// <returns></returns>
        public static IMonoid<Lst<X>> List<X>()
            => make(ListModule.eq, ListModule.concat, ListModule.zero<X>());

        /// <summary>
        /// Constructs a seq monoid
        /// </summary>
        /// <typeparam name="X"></typeparam>
        /// <returns></returns>
        public static IMonoid<Seq<X>> Seq<X>()
            => make(SeqModule.eq, SeqModule.concat, SeqModule.empty<X>());
