...
namespace SqlT.Models
{
    ...
    public sealed class SqlProxySelectBuilder<P> : SqlSelectBuilder<SqlProxySelectBuilder<P>, P>
       where P : ISqlTabularProxy, new()
    {
        ...
        public SqlProxySelectBuilder()
        {
            Columns(Tabular.Columns.Select(c => c.ColumnName));
        }

        public Builder<SqlProxySelectBuilder<P>> OrderBy(Expression<Func<P, object>> selector, sx.sort_direction direction = null)
        {
            var orderby = new sort_column<P>(selector, direction ?? sx.DESC);
            return this;
        }

        public Builder<SqlProxySelectBuilder<P>> OrderBy(params Expression<Func<P, object>>[] selectors)
        {
            var orderbys = map(selectors, selector => new sort_column<P>(selector, sx.DESC));
            return this;
        }

        public Builder<SqlProxySelectBuilder<P>> OrderBy(sx.sort_direction direction, params Expression<Func<P, object>>[] selectors)
        {
            var orderbys = map(selectors, selector => new sort_column<P>(selector, direction));
            return this;
        }

        public Builder<SqlProxySelectBuilder<P>> And<C>(Expression<Func<P, C>> selector)
        {
            return this;
        }

        public Builder<SqlProxySelectBuilder<P>> OrderBy<C>(Expression<Func<P, C>> selector,
