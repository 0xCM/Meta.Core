...
namespace SqlT.Syntax
{
    ...
    public static class SqlTypeMapper
    {
        ...
        public static Option<CoreDataType> MapToCoreType(this SqlTypeName typeName)
            => typeName.GetSqlType().MapToCoreType();

        public static Option<CoreDataType> MapToCoreType(this sxc.ISqlType src)
            => src is sxc.native_type ? SqlToCoreTypeMap.Map(src)
            : (src is sxc.ISqlDataType d 
                ? SqlToCoreTypeMap.Map(d.BaseType.value) 
            : none<CoreDataType>());

        public static Option<ClrType> MapToClrType(this sxc.ISqlType src)
            => from x in src.MapToCoreType() select ClrType.Get(x.ClrType);

        public static ClrType MapToTransportType(this sxc.ISqlType src)
            => ClrType.Get(SqlToTransportTypeMap.Map(src));

        public static Option<object> ConvertToTransportValue(this sxc.ISqlType srcType, object srcValue)
           => Try(() =>
               {
                   if (srcValue == null)
                       return DBNull.Value;
                   ...
                   if (ReferenceEquals(srcType, SqlNativeTypes.date) && srcValueType == typeof(Date))
        ...
        public static Option<object> ConvertToTransportValue(this CoreDataType srcType, object srcValue)
