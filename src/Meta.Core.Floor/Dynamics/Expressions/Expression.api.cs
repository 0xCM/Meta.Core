...
partial class metacore
{
    ...
    /// <param name="f"></param>
    /// <returns></returns>
    public static Expression<Func<T>> express<T>(Func<T> f)
    ...
    /// <param name="f"></param>
    /// <returns></returns>
    public static Expression<Func<X, Y>> express<X, Y>(Func<X, Y> f)
        => FuncExpression.make(f);
    ...
    /// <param name="f"></param>
    /// <returns></returns>
    public static Expression<Func<X1, X2, Y>> express<X1, X2, Y>(Func<X1, X2, Y> f)
        => FuncExpression.make(f);
    ...
    /// <param name="f"></param>
    /// <returns></returns>
    public static Expression<Func<X1, X2, X3, Y>> express<X1, X2, X3, Y>(Func<X1, X2, X3, Y> f)
        => FuncExpression.make(f);
    ...
    public static InvocationExpression invocation<X, Y>(Func<X, Y> f, string argName)
        => Expression.Invoke(express(f), paramX<X>(argName));
    ...
    public static InvocationExpression invocation<X1, X2, Y>(Func<X1, X2, Y> f, string arg1 = "x1", string arg2 = "x2")
        => Expression.Invoke(express(f), paramX<X1>(arg1), paramX<X2>(arg2));
    ...
    /// <returns></returns>
    public static Func<X> func<X>(MethodInfo m, object instance = null)
        => (Func<X>)_funcCache.GetOrAdd(m, method =>
        {
    ...
    public static Option<Func<X1, X2>> func<X1, X2>(Type declarer, string name, object instance = null)
        => from m in method(declarer, name, typeof(X1))
    ...
    /// <param name="instance">The instance of the declaring type, if method is not static</param>
    /// <returns></returns>
    public static Func<X1, X2, Y> func<X1, X2, Y>(MethodInfo m, object instance = null)
        => (Func<X1, X2, Y>)_funcCache.GetOrAdd(m, method =>
        {
    ...
    public static class linqx
    {
        ...
        public static Option<Func<X1, X2, bool>> and<X1, X2>(Func<X1, bool> p1, Func<X2, bool> p2)
            => from args in some(paramsX<X1,X2>())
               let left = invoke(express(p1), args.x1)
               let right = invoke(express(p2), args.x2)
