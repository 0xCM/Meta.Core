...
namespace SqlT.Core
{
    ...
    public class SqlColumnReader<X1, X2, X3, X4> : SqlColumnReader, ISqlColumnReader<X1, X2, X3, X4>
    {
        ...
        IEnumerator<DataFrameRow<X1, X2, X3, X4>> GetEnumerator()
        {
            foreach (DataFrameRow<X1, X2, X3, X4> col in GetColumnData())
                yield return col;
        }

        IEnumerator<DataFrameRow<X1, X2, X3, X4>> IEnumerable<DataFrameRow<X1, X2, X3, X4>>.GetEnumerator()
            => GetEnumerator();
