...
namespace SqlT.Core
{
    ...
    public static class SqlDataFrameX
    {
        public static ISqlDataset<T> ToDataset<T>(this IEnumerable<T> proxies)
            where T : class, ISqlTabularProxy, new()
                => new SqlDataset<T>(new SqlDatasetDesignator(new SqlContentType(typeof(T).Name)), proxies);
